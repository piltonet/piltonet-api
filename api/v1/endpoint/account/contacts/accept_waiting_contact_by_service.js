...
class leaf { // Required
  constructor(){
    this.__uri__ = '/account/contacts/accept_waiting_contact_by_service'; // Required: Defining new leaf under /v1/
    this.__path__ = 'v1/endpoint/acounts/contacts/accept_waiting_contact_by_service'; // Optional: path of file
    ...
    this.__methods__ = { // Methods and Resposible Functions
      'post': acceptWaitingContactByService // Setting for each http method a function
    };
  }
}
...
/*********************************************************************************************************************/
/*********************************************************************************************************************/
async function acceptWaitingContactByService(http_request, response){
  ...
  try{
    var params = libs.validations.validate_request_params(http_request, ['profile_tba', 'contact_tba', 'contact_id'], []);
  }catch(e){
  ...
  // To Do
  /***************** ERC1155Contacts > addContactByOwner *******************/
  try {
    const NETWORK = process.env.DEFAULT_NETWORK;
    const deployedERC1155Contacts = require.main.require(`./contracts/deployments/${NETWORK}/ERC1155Contacts.json`);
  
    const contractAbi = require.main.require("./contracts/abi/ERC1155Contacts.json");
    const ERC1155Contacts = await ethers.getContractAt(contractAbi, deployedERC1155Contacts.address);
  
    const tx = await ERC1155Contacts.addContactByOwner(params.verifiedParams.profile_tba, params.verifiedParams.contact_tba, {
      gasLimit: 4000000
    });
    await tx.wait()
