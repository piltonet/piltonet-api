...
class leaf { // Required
  constructor(){
    this.__uri__ = '/account/link/accept'; // Required: Defining new leaf under /v1/
    this.__path__ = 'v1/endpoint/account/link_accept'; // Optional: path of file
    ...
    this.__methods__ = { // Methods and Resposible Functions
      'post': linkAccept // Setting for each http method a function
    };
  }
}
...
/*********************************************************************************************************************/
/*********************************************************************************************************************/
async function linkAccept(http_request, response){
  ...
  try{
    var params = libs.validations.validate_request_params(http_request, ['waiting_account_address'], []);
  }catch(e){
  ...
  let where_params = {
    account_address: params.verifiedParams.waiting_account_address,
    account_status: 'waiting',
    main_account_address: Account.account_address
  }
  let account_update = await models.queries.update_table('accounts', {account_status: 'linked'}, where_params);
  ...
  resp.message = 'The link request was successfully accepted.';
  resp.result = [];
  response.status(200);
  response.json(resp);
  return
